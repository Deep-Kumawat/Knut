...
# 22.06.22
# We need to modify the macro feature such that, users can map a macro key to a keyboard shortcut rather than be stuck with using a single key.
################
# Notes From Zaid
################
...
import pickle
# end of imports-----------------------
# i want to somehow store the action in a list or something
...
while userChoiceToMakeNewMacrosOrUseExisting != 0:
    if userChoiceToMakeNewMacrosOrUseExisting == 0:  # 0 to exit
        ...
        #new variable ---> userChoice_MultiKeyOrSingleKey
        userChoice_MultiKeyOrSingleKey = (input("1.Single-key macro\n2.Multi-key macro\n(To know the difference between the two enter '?')"))
        if userChoice_MultiKeyOrSingleKey == '?':
            print("here is the difference bbetween  the two")#this is just a place holder
        # userChoice_MultiKeyOrSingleKey = int(userChoice_MultiKeyOrSingleKey)
        elif int(userChoice_MultiKeyOrSingleKey) == 1:
        ...
        elif int(userChoice_MultiKeyOrSingleKey) == 2:
            print("press the keys one at a time to which you want to map the action and then press the escape key:")
            time.sleep(0.5)#it was registering enter without this delay
            inputMultiMacroKey = keyboard.record(until='esc')
            print(inputMultiMacroKey)
            #we need to clear the unwanted variables from inputmacrokey list
            newInputMacroKey = []
            for eventIndex in range(1, len(inputMultiMacroKey), 2):
                
                newInputMacroKey.append(str(inputMultiMacroKey[eventIndex])[14:-4])
            print("this is the newinputmacrokey", newInputMacroKey)
            # print("Your selected macro key was --->", inputMultiMacroKey)
            pickle.dump(inputMultiMacroKey, open("inputMacroKeyStoringFile.dat", "wb"))
